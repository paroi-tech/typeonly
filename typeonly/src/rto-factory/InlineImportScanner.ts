...
import type AstImportTool from "./AstImportTool.js";
...
export default class InlineImportScanner {
  ...
  scan(astNode: AstType): void {
    ...
    if (!scan) throw new Error(`Unexpected whichType to scan: ${astNode.whichType}`);
    scan(astNode);
  }
  ...
  private scanTupleType(astNode: AstTupleType) {
    if (astNode.itemTypes) astNode.itemTypes.forEach((child) => this.scan(child));
  }

  private scanFunctionType(astNode: AstFunctionType) {
    this.scan(astNode.returnType);
    if (astNode.parameters) this.scanFunctionParameters(astNode.parameters);
    if (astNode.generic) this.scanGenericParameters(astNode.generic);
  }

  private scanFunctionParameters(astNodes: AstFunctionParameter[]) {
    return astNodes.forEach(({ type }) => {
      if (type) this.scan(type);
    });
  }

  private scanGenericParameters(astNodes: AstGenericParameter[]) {
    return astNodes.forEach(({ extendsType, defaultType }) => {
      if (extendsType) this.scan(extendsType);
      if (defaultType) this.scan(defaultType);
    });
  }

  private scanInterface(astNode: AstInterface) {
    if (!astNode.entries) return;
    for (const entry of astNode.entries) {
      if (entry.whichEntry === "indexSignature") this.scanIndexSignature(entry);
      else if (entry.whichEntry === "mappedIndexSignature") this.scanMappedIndexSignature(entry);
      else if (entry.whichEntry === "property") this.scanProperty(entry);
  ...
  private scanPropertyFromFunctionProperty(entry: AstFunctionProperty) {
    if (entry.parameters) this.scanFunctionParameters(entry.parameters);
    if (entry.returnType) this.scan(entry.returnType);
    if (entry.generic) this.scanGenericParameters(entry.generic);
  }
